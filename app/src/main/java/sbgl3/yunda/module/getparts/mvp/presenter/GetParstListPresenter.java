...
@ActivityScope
public class GetParstListPresenter extends BasePresenter<GetParstListContract.Model, GetParstListContract.View> {
    @Inject
    ...
    @Inject
    public GetParstListPresenter(GetParstListContract.Model model, GetParstListContract.View rootView) {
                ...
                .subscribe(new Observer<BaseResponsBean<List<GetPartsBean>>>() {
                        ...
                        if(isScan){
                            if(baseResponsBean!=null&&baseResponsBean.getData()!=null){
                                ...
                                if(baseResponsBean!=null&&baseResponsBean.getMessage()!=null){
                                    mRootView.OnLoadFaild("扫码获取待修未接收配件信息失败"+baseResponsBean.getMessage());
                                }else {
                                    mRootView.OnLoadFaild("扫码获取待修未接收配件信息失败,请重试");
                                }
                            }
                        }else {
                            if(baseResponsBean!=null&&baseResponsBean.getData()!=null){
                                if(baseResponsBean.getData().size()>0){
                                    ...
                                    mRootView.OnLoadFaild("无相关待修已接收配件信息");
                                }
                            }else {
                                if(baseResponsBean!=null&&baseResponsBean.getMessage()!=null){
                                   mRootView.OnLoadFaild("获取待修已接收配件信息失败"+baseResponsBean.getMessage());
                                }else {
                                    mRootView.OnLoadFaild("获取待修已接收配件信息失败,请重试");
                                }
                            }
                        }
                    ...
                    public void onError(Throwable e) {
                        ...
                        if(isScan){
                            ...
                            mRootView.OnLoadFaild("获取待修已接收配件信息失败,请重试"+e.getMessage());
                        }
                    }
